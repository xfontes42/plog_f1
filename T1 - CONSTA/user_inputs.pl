...
seleciona_local(X,Y) :-
    repeat,
      ...
      board_size(Size), LimitX is Size+17,
      it((Temp_X @>= LimitX; Temp_X@=<16),(write('Coordenada X de Board invalida!'),nl,fail)),
      X is Temp_X - 17,
      LetraX is Temp_X+48,
    repeat,
      ...
      once(le_numero(Y)),
      LimitY is Size,
      it((Y @>= LimitY; Y@<0 ),(write('Coordenada Y de Board invalida!'),nl,fail)),
      LetraY is Y,
      write('Coordenadas selecionadas ('),put_code(LetraX),write(', '),write(LetraY),write(').'),nl.
